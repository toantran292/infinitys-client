...
const SOCKET_URL = process.env.NEXT_PUBLIC_SOCKET_URL || 'http://localhost:20250/notifications';
...
export function NotificationProvider({ children }: { children: ReactNode }) {
    ...
    const handleNewNotification = (newNotification: NotificationWsResponse) => {
        switch (newNotification.event_name) {
            case 'friend_request:received':
                toast.custom((t) => (
                    <FriendRequestToast {...newNotification.meta} message="Đã gửi cho bạn lời mời kết bạn" type="friend_request" t={t} />
                ));
                break;
    ...
    useEffect(() => {
        ...
        socket.on('notifications.new', (newNotification: NotificationWsResponse) => {
            // queryClient.setQueryData(['notifications'], (oldData: Notification[] = []) =>
