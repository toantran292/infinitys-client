...
export const ProblemsIdView = () => {
  ...
  const { data: summary, isLoading: isLoadingSummary } = useQuery({
    ...
    queryFn: async () => {
      const response = await axiosInstance.get(
        `/api/problems/${id}/submissions/summary`
      );
  ...
  const { mutate: submitCode, isPending: isRunning } = useMutation({
    ...
    onSuccess: () => {
      toast.success(
        "Code submitted successfully. Click refresh to see results."
      );
    },
    onError: () => {
      ...
      <div className="flex h-[calc(100vh-72px)]">
                    ...
                    <div className="mb-6 bg-gray-50 p-4 rounded-lg">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div>
                          ...
                          <div className="text-lg font-semibold">
                            {summary?.bestRuntime
                              ? formatRuntime(summary.bestRuntime)
                              : "-"}
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="text-lg font-semibold">
                            {summary?.bestMemory
                              ? formatMemory(summary.bestMemory)
                              : "-"}
                          </div>
                        </div>
                      </div>
                            ...
                            <div className="p-4">
                              <div className="space-y-2">
                                ...
                                <div>
                                  ...
                                  <span className="ml-2">
                                    {formatMemory(result.failedTestCase.memory)}
                                  </span>
                                </div>
                              </div>
                            </div>
        ...
        <div className="flex-1 flex flex-col">
          <div className="p-4 border-b flex justify-between items-center">
            <select className="border rounded px-3 py-1" defaultValue="cpp">
