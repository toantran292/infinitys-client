...
const formSchema = z.object({
    ...
    description: z.string().min(1, "Vui lòng nhập mô tả công việc").max(10000, "Mô tả không được vượt quá 10000 ký tự"),
});
...
export default function CreateJobForm() {
    ...
    const { data: companies, isLoading: isLoadingCompanies } = useQuery<Page[]>({
        ...
        queryFn: () => axiosInstance.get('/api/pages/me').then(res => res.data.items)
    });
    ...
    const { mutate: createJob, isPending: isSubmitting } = useMutation({
        mutationFn: (data: JobFormData) =>
            axiosInstance.post("/api/recruitment-posts", data).then(res => res.data),
        onSuccess: (data) => {
            toast.success("Tạo bài tuyển dụng thành công!");
            router.push(`/jobs/${data.id}`);
        },
        onError: (error: AxiosError<ErrorResponse>) => {
            toast.error(error.response?.data?.message || "Có lỗi xảy ra khi tạo bài tuyển dụng");
        }
    });
        ...
        <div className="min-h-screen flex flex-col bg-gray-50">
                            ...
                            <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                                <div className="p-8 space-y-6">
                                    <div className="grid gap-6 md:grid-cols-2">
                                        ...
                                        {/* Job Position */}
                                        <div className="space-y-2 md:col-span-2">
                                            <Label htmlFor="jobPosition" className="text-sm font-medium flex items-center gap-1">
                                                Vị trí công việc
                                                <span className="text-red-500">*</span>
                                            </Label>
                                            <Controller
                                            ...
                                            <div className="p-4 bg-blue-50 rounded-lg mb-4 flex items-center gap-2">
                                                <div className="p-1.5 rounded-full bg-blue-100">
                                                    <svg className="w-4 h-4 text-blue-600" viewBox="0 0 24 24">
                                                        <path fill="currentColor" d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-1-4h2v2h-2zm1-10c-2.21 0-4 1.79-4 4h2c0-1.1.9-2 2-2s2 .9 2 2c0 2-3 1.75-3 5h2c0-2.25 3-2.5 3-5 0-2.21-1.79-4-4-4z" />
                                                    </svg>
                                                </div>
                                                <p className="text-sm text-blue-700 flex-1">
                                                    Tạo bài tuyển dụng chất lượng cao bằng cách sử dụng hướng dẫn đề xuất dưới đây.
                                                    <button className="ml-1 font-medium hover:underline">Tìm hiểu thêm</button>
                                                </p>
                        ...
                        {companies && (
                            <div className="w-[400px] sticky top-8">
                                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                                    <h2 className="text-base font-semibold mb-4">Bài tuyển dụng</h2>
                                    <JobPreviewCard
                                        title={formValues.title || "Tiêu đề tuyển dụng"}
                                        company={companies.find((c: Page) => c.id === formValues.pageId)?.name || "Tên công ty"}
                                        ...
                                        avatar={companies.find((c: Page) => c.id === formValues.pageId)?.avatar?.url || "https://github.com/shadcn.png"}
                                    />
                                </div>
                            </div>
                        )}
            ...
            >
                <DialogContent className="bg-white">
                    <DialogHeader>
                        <DialogTitle className="text-gray-900">Chỉ công ty mới có thể đăng tuyển dụng</DialogTitle>
                        <DialogDescription className="text-gray-600">
                            Bạn cần có ít nhất một công ty để đăng bài tuyển dụng. Vui lòng tạo trang công ty trước khi đăng bài.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="flex justify-end">
                        <Button onClick={() => router.push('/jobs')} className="bg-black hover:bg-gray-800 text-white px-6">
                            Trở về
                        </Button>
                    </div>
                </DialogContent>
