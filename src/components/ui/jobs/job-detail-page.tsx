        ...
        <div className="container max-w-[1200px] mx-auto px-4 py-8">
          <div className="flex gap-8">
            ...
            <div className="flex-1 max-w-[780px] space-y-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 mt-6 justify-end">
                      <Button
                        ...
                        onClick={handleApply}
                        disabled={
                          !job.active || applyMutation.isPending || application
                        }
                      >
                        {!job.active
                          ? "Hết hạn ứng tuyển"
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div className="mt-6 pt-6 border-t border-gray-200">
                  <p className="text-sm text-gray-500 text-right">
                    Được đăng bởi {author.lastName} {author.firstName} •{" "}
                    {timeAgo}
                  </p>
                </div>
              </div>
              ...
              {/* Applicants List */}
              {job?.pageUser.user.id === user?.id &&
                (job.pageUser.role === "ADMIN" ||
                  job.pageUser.role === "OPERATOR") && (
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                  ...
                  >
                    <div className="w-12 h-12 relative rounded-lg overflow-hidden bg-gray-100">
                      <img
                        src={
                          page?.avatar?.url || "https://github.com/shadcn.png"
                        }
                        alt="Logo"
                        className="object-cover"
                      />
...
function ApplicantsList({ jobId }: { jobId: string }) {
  ...
  const { data: response, isLoading } = useQuery<{
    ...
    queryFn: () =>
      axiosInstance
        .get(
          `/api/recruitment-posts/${jobId}/applications?page=${currentPage}&take=${take}`
        )
        .then((res) => res.data)
  });
